...
class Auditing(Observer):
    def update(self, account, message):
        print(
            f"AUDIT - Account {account.account_number} modified: {message}\n"
        )
...
class SavingsAccount(Account):
    def withdraw(self, amount):
        if 0 < amount <= self._balance:
            ...
            self.notify(
                f"Withdrew {amount} ETB. New balance: {self._balance} ETB"
            )
        else:
...
class AccountRegistry:
    ...
    def top_by_balance(self, n=5):
        accts = sorted(
            self.by_number.values(), key=lambda a: a.balance, reverse=True
        )
        return accts[:n]
    ...
    def undo_last(self, number):
        ...
        if last_tr["type"] == "deposit":
            ...
            account.notify(
                f"Reversed deposit of {last_tr['amount']} ETB. New balance: {account.balance} ETB"
            )
        elif last_tr["type"] == "withdraw":
            account._balance += last_tr["amount"]
            account.notify(
                f"Reversed withdrawal of {last_tr['amount']} ETB. New balance: {account.balance} ETB"
            )
...
saving2 = AccountFactory.create("saving", "Marta", "SA10009999", 5000)
current2 = AccountFactory.create("current", "Aster", "CU10008888", 25000)

head_office = Branch("National head office")
regional_branch = Branch("Regional branch office")
bole_branch = Branch("Bole local branch")

head_office.add_child(regional_branch)
regional_branch.add_child(bole_branch)

head_office.add_account(saving1)  
regional_branch.add_account(current1)  
bole_branch.add_account(saving2) 
bole_branch.add_account(current2)

print(f"Bole Branch Total: {bole_branch.total_balance()} ETB")
print(f"Total Bank System Capital Network: {head_office.total_balance()} ETB")

transfers_graph = {
    "SA10001234": ["CU10005678", "SA10009999"],  
    "CU10005678": ["CU10008888"],  
    "SA10009999": [], 
    "CU10008888": ["SA10001234"], 
}

# Run path analysis check using BFS
starting_point = "SA10001234"
recipients_network = bfs_transfers(transfers_graph, starting_point)
print(f"\nAccounts reachable via transfers from {starting_point}: {recipients_network}")
